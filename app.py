...
# --- 1. ตั้งค่าหน้ากระดาษ ---
st.set_page_config(page_title="ระบบจัดการนักศึกษา - Final Fix", layout="wide")
...
# --- 3. ส่วนกรอกข้อมูล ---
st.title("📑 ระบบจัดการรายชื่อวิทยาลัยเทคโนโลยีนนทบุรี")
...
def create_excel_report(target_year):
    ...
    for r_name in sorted(year_data['Room'].unique()):
        ...
        # ปรับความกว้างคอลัมน์ (แก้จุดที่เคย Error)
        ...
        # แทรกโลโก้ (ถ้ามีไฟล์ logo.jpg อยู่ในเครื่อง)
...
with c1:
    ...
    if f1: st.download_button("📥 โหลดใบรายชื่อ ปี 1", f1, "ปี1.xlsx", use_container_width=True)
with c2:
    f2 = create_excel_report("ปี2")
    if f2: st.download_button("📥 โหลดใบรายชื่อ ปี 2", f2, "ปี2.xlsx", use_container_width=True)
