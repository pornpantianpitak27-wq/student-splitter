...
st.title("📑 ระบบออกใบรายชื่อและเช็คชื่อ (ตามรูปแบบ Excel จริง)")
...
def create_excel_report(target_year):
    ...
    for r_name in sorted(year_data['Room'].unique()):
        ...
        # --- ตารางขวาบน (บัญชีรายชื่อนี้ใช้สำหรับ...) ---
        ...
        for i in range(1, 9): ws.cell(row=10, column=12+i).value = i # ตัวเลขคาบ 1-8
        ...
        # --- แทรกโลโก้ (ตั้งชื่อไฟล์รูปว่า 1523.jpg) ---
        if os.path.exists("1523.jpg"):
            img = Image("1523.jpg")
            img.height = 75; img.width = 75 # ปรับขนาดให้สวยงาม
            ws.add_image(img, 'H1') # วางตำแหน่งกลางไฟล์แถวบน
